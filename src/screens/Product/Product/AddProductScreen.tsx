...
const AddProductScreen = ({navigation}: any) => {
  ...
  const [selectedCategory, setSelectedCategory] = useState<IndexPath>(
    new IndexPath(0),
  );
    ...
    <SafeAreaView>
      ...
      <ScrollView className="p-4">
        <View className="flex flex-col space-y-3 mt-4 mb-4 p-2 border border-orange-500 rounded-xl border-dashed">
          <Text className="text-lg font-bold mb-2 text-gray-600">
            Upload Images
          </Text>
          <TouchableOpacity
            className="flex flex-row justify-center items-center space-x-2 bg-gray-200 p-4 rounded-lg border border-orange-400 border-dotted"
            onPress={pickImage}>
        ...
        <Dialog
          ...
          <Dialog.Content>
            <Text className="text-center text-green-600">
              Congratulation! You have successfully added a new product!
            </Text>
          </Dialog.Content>
        </Dialog>
