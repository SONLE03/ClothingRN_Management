...
const DailyReportScreen = ({navigation}: any) => {
  ...
  const fetchData = async () => {
    try {
      const revenueResponse = await GetDailyRevenue(
        startDate.toISOString(),
        endDate.toISOString(),
      );
      const expenseResponse = await GetDailyExpense(
        startDate.toISOString(),
        endDate.toISOString(),
      );
      console;
  ...
  const renderCard = (title: string, value: number) => (
    <Card
      className="border border-orange-600 rounded-xl p-2"
        ...
        <Tab title="Expense">
          <ScrollView style={{padding: 16}}>
            <View className="flex flex-col space-y-4">
              {renderCard(
                'Total Invoices',
                calculateTotal(expenseData, 'totalInvoices'),
              )}
              {renderCard(
                'Total Products',
                calculateTotal(expenseData, 'totalProducts'),
              )}
              {renderCard(
                'Total Expense',
                calculateTotal(expenseData, 'totalExpense'),
              )}
            </View>
